...
fn benchmark_insert(c: &mut Criterion) {
    c.bench_function("insert", |b| b.iter_with_setup(|| {
        ...
        for id in ids {
            db.execute_query(&Query::tuple_untyped(&[&id, "12", "example_doc", "the content"]).insert_into("document")).unwrap();
        }
    }));
}
...
fn benchmark_filter(c: &mut Criterion) {
    let db = create_database();
    for i in 1..100000 {
        db.execute_query(&Query::tuple_untyped(&[i.to_string().as_str(), "12", "example_doc", "the content"]).insert_into("document")).unwrap();
    }
...
fn benchmark_count(c: &mut Criterion) {
    ...
    for i in 0..1_000_000 {
        let id = i.to_string();
        let query = Query::tuple_untyped(&[&id, "2", "example_doc", "the_content"]).insert_into("document");
...
fn benchmark_query_to_string(c: &mut Criterion) {
    ...
    let insert_query = Query::tuple_untyped(&["1", "foo", "bar"]).insert_into("document");
